...
class DocumentProcessor:
    ...
    def _safe_load_file(self, file_path: Path, loader_class):
        ...
        try:
            logger.info(
                f"Loading file: {file_path.name} ({file_path.stat().st_size / 1024 / 1024:.1f} MB)"
            )
            ...
            if (
                ...
                logger.warning(
                    f"Large PDF detected: {file_path.name}. Processing with special handling..."
                )
    ...
    def split_documents(self, documents):
        ...
        try:
            ...
            if len(valid_chunks) != len(all_chunks):
                logger.warning(
                    f"Removed {len(all_chunks) - len(valid_chunks)} empty chunks"
                )
    ...
    def process_documents(self, path: Optional[str] = None):
        ...
        try:
            ...
            logger.info(
                f"Document processing completed: {len(chunks)} chunks ready for indexing"
            )
            return chunks
