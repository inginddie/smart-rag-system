...
class VectorStoreManager:
    ...
    def _reset_vector_store(self):
        ...
        try:
            ...
            if os.path.exists(self.persist_directory):
                ...
                logger.info(
                    f"Removed existing vector store at: {self.persist_directory}"
                )
    ...
    def vector_store(self):
        ...
        if self._vector_store is None:
            ...
            for attempt in range(max_retries):
                try:
                    logger.info(
                        f"Initializing vector store (attempt {attempt + 1}/{max_retries})"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Vector store initialized successfully at: {self.persist_directory}"
                        )
                        break
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise VectorStoreException(
                            f"Failed to initialize vector store after {max_retries} attempts: {e}"
                        )
            ...
            for i in range(0, len(documents), batch_size):
                ...
                try:
                    ...
                    if clean_batch:
                        ...
                        all_ids.extend(ids)
                        logger.info(
                            f"Successfully added batch: {len(clean_batch)} documents"
                        )
                    else:
    ...
    def load_and_index_documents(self, documents_path: Optional[str] = None) -> int:
        ...
        try:
            ...
            logger.info(
                f"Found {file_info.get('total_files', 0)} files ({file_info.get('total_size_mb', 0):.1f} MB total)"
            )
    ...
    def similarity_search(self, query: str, k: int = 5, intent_type=None):
        ...
        try:
            ...
            if settings.enable_query_expansion:
                try:
                    ...
                    if expansion_result.expansion_count > 0:
                        logger.info(
                            f"Query expanded: {expansion_result.expansion_count} terms added"
                        )
            ...
            logger.debug(
                f"Found {len(results)} similar documents for {'expanded' if expansion_info and expansion_info['expansion_count'] > 0 else 'original'} query"
            )
            return results
        ...
        try:
            ...
            if hasattr(vs, "_collection") and vs._collection:
                try:
                    ...
                    # Alternative method for ChromaDB
                    try:
                        results = vs._collection.get(limit=1)
                        if results and "ids" in results and results["ids"]:
                            # If we can get at least one document, collection is not empty
                            # Get approximate count by querying with high limit
                            full_results = vs._collection.get()
                            count = (
                                len(full_results.get("ids", [])) if full_results else 0
                            )
                    except:
                        count = 0
