...
class UsageAnalytics:
    ...
    def track_query_outcome(
        ...
        try:
            ...
            self.effectiveness_by_intent[outcome.intent_type].append(
                effectiveness_score
            )
            ...
            logger.debug(
                f"Tracked query outcome: {effectiveness_score:.3f} for {intent_type}"
            )
    ...
    def get_successful_patterns(
        self, intent_type: IntentType, min_effectiveness: float = 0.7
    ) -> List[QueryPattern]:
        ...
        try:
            ...
            successful_queries = [
                ...
                if outcome.intent_type == intent_str
                and outcome.effectiveness_score >= min_effectiveness
            ]
        ...
        try:
            ...
            for intent_type in [
                ...
                failed_queries = [
                    outcome
                    for outcome in self.query_outcomes
                    if outcome.intent_type == intent_type
                    and outcome.effectiveness_score < 0.5
                ]
    ...
    def get_analytics_summary(self) -> Dict:
        ...
        try:
            ...
            avg_effectiveness = (
                sum(o.effectiveness_score for o in self.query_outcomes) / total_queries
            )
            ...
            for intent_type, scores in self.effectiveness_by_intent.items():
                if scores:
                    intent_stats[intent_type] = {
                        ...
                        "success_rate": len([s for s in scores if s >= 0.7])
                        / len(scores),
                    }
    ...
    def _extract_query_patterns(
        self, successful_queries: List[QueryOutcome]
    ) -> List[QueryPattern]:
        ...
        for structure, outcomes in structure_groups.items():
            if len(outcomes) >= self.min_samples_for_pattern:
                pattern = QueryPattern(
                    ...
                    avg_effectiveness=sum(o.effectiveness_score for o in outcomes)
                    / len(outcomes),
    ...
    def _extract_query_structure(self, query: str) -> str:
        ...
        if query_lower.startswith(("¿qué es", "what is")):
            ...
            word in query_lower for word in ["compara", "compare", "vs", "versus"]
        ):
            return "comparison_pattern"
        elif any(
            word in query_lower
            for word in ["estado del arte", "state of art", "tendencias"]
        ):
            return "state_of_art_pattern"
        elif any(
            word in query_lower for word in ["limitaciones", "gaps", "oportunidades"]
        ):
            ...
            issues.append(
                {
                    ...
                    "frequency": len([l for l in query_lengths if l < 5])
                    / len(query_lengths),
    ...
    def _load_analytics(self) -> None:
        ...
        try:
            ...
            self.suggestion_adoptions = defaultdict(
                int, data.get("suggestion_adoptions", {})
            )
            ...
            logger.debug(
                f"Loaded {len(self.query_outcomes)} query outcomes from analytics"
            )
