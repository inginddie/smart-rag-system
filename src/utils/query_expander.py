...
class QueryExpander:
    ...
    def expand_query(
        self, query: str, intent_type: Optional[IntentType] = None
    ) -> ExpansionResult:
        ...
        try:
            ...
            final_query = self._build_final_query(
                query, filtered_expansions, protected_terms
            )
            ...
            logger.debug(
                f"Query expansion completed: {len(filtered_expansions)} terms added in {processing_time:.1f}ms"
            )
            return result
    ...
    def _extract_expansion_candidates(
        self, query: str, protected_terms: List[str]
    ) -> List[str]:
    ...
    def _generate_expansions(
        self, candidates: List[str], intent_type: Optional[IntentType]
    ) -> List[str]:
        ...
        for candidate in candidates:
            ...
            if candidate.upper() in self.vocabulary.acronym_expansions:
                acronym_expansions = self.vocabulary.acronym_expansions[
                    candidate.upper()
                ]
    ...
    def _build_final_query(
        self, original_query: str, expansions: List[str], protected_terms: List[str]
    ) -> str:
