...
class AcademicQualityValidator:
    ...
    def validate_response(self, 
                         ...
                         template_metadata: TemplateMetadata) -> QualityScore:
        ...
        return QualityScore(
            ...
            quality_criteria={criterion: 1.0 for criterion in template_metadata.quality_criteria},
            issues_found=issues,
    ...
    def _assess_content_depth(self, response: str, intent_type: IntentType) -> float:
        ...
        depth_indicators = {
            IntentType.DEFINITION: [
                "características", "atributos", "propiedades",
                "definición", "concepto", "término"
            ],
            IntentType.COMPARISON: [
                "ventajas", "desventajas", "diferencias", "similitudes",
                "comparación", "contraste", "versus"
            ],
            IntentType.STATE_OF_ART: [
                "tendencias", "enfoques", "métodos", "estado actual",
                "evolución", "desarrollo"
            ],
            IntentType.GAP_ANALYSIS: [
                "limitaciones", "gaps", "oportunidades", "futuro",
                "investigación", "challenges"
            ]
        }
    ...
    def _assess_clarity(self, response: str) -> float:
        ...
        transition_words = [
            "además", "sin embargo", "por otro lado", "en consecuencia",
    ...
    def _has_clear_organization(self, response: str) -> bool:
        ...
        found_patterns = sum(1 for pattern in organization_indicators 
    ...
    def _identify_issues(self, response: str, metadata: TemplateMetadata) -> List[str]:
        ...
        if not any(re.search(pattern, response) for pattern in self.citation_patterns):
            issues.append("No citations found - academic responses should include references")
        ...
        if missing_sections:
            issues.append(f"Missing expected sections: {', '.join(missing_sections[:2])}")
    ...
    def _generate_recommendations(self, scores: Dict[str, float]) -> List[str]:
        ...
        if scores["clarity"] < 0.7:
            recommendations.append("Improve clarity with better transitions and sentence structure")
