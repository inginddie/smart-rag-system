...
class QueryPreprocessor:
    ...
    def _extract_linguistic_features(self, text: str) -> LinguisticFeatures:
        ...
        question_words = [
            ...
            if any(
                word.startswith(qw)
                for lang_qw in self.question_words.values()
                for qw in lang_qw
            )
        ]
        ...
        academic_verbs = [
            word for word in words if any(verb in word for verb in self.academic_verbs)
        ]

        # Encontrar marcadores de comparación
        comparison_markers = [
            marker for marker in self.comparison_markers if marker in text
        ]

        # Encontrar indicadores temporales
        temporal_indicators = [
            indicator for indicator in self.temporal_indicators if indicator in text
        ]
...
class KeywordBasedClassifier:
    ...
    def _compile_patterns(self):
        ...
        self.compiled_patterns[IntentType.STATE_OF_ART] = [
            (
                re.compile(r"\b(?:state\s+of\s+(?:the\s+)?art|estado\s+del\s+arte)\b"),
                0.95,
            ),
    ...
    def classify(self, query: str, features: LinguisticFeatures) -> IntentResult:
        ...
        reasoning = self._generate_reasoning(
            best_intent, matched_patterns[best_intent], confidence
        )
    ...
    def _calculate_intent_score(
        self, query: str, intent_type: IntentType, features: LinguisticFeatures
    ...
    def _generate_reasoning(
        self, intent_type: IntentType, patterns: List[str], confidence: float
    ) -> str:
    ...
    async def detect_intent(self, query: str) -> IntentResult:
        ...
        try:
            ...
            if total_time > settings.intent_max_processing_time_ms:
                logger.warning(
                    f"Intent detection tardó {total_time:.1f}ms, excediendo SLA de {settings.intent_max_processing_time_ms}ms"
                )

            # Log para debugging y métricas
            logger.debug(
                f"Intent detected: {result.intent_type.value} (confidence: {result.confidence:.2f}, time: {result.processing_time_ms:.1f}ms)"
            )
            ...
            logger.error(
                f"Error en detección de intención para query '{query[:50]}...': {e}"
            )
