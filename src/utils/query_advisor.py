...
class QueryAdvisor:
    ...
    def analyze_query_effectiveness(
        self, query: str, result: Dict, intent_result: Optional[IntentResult] = None
    ) -> EffectivenessScore:
        ...
        try:
            ...
            logger.debug(
                f"Effectiveness analysis completed in {processing_time:.1f}ms: {total_score:.3f}"
            )
    ...
    def get_contextual_tips(
        self, intent_type: IntentType, complexity_score: float = 0.5
    ) -> List[ContextualTip]:
    ...
    def _generate_effectiveness_reasoning(
        ...
        if total_score >= 0.8:
            ...
            return (
                "Consulta necesita reformulación para mejorar relevancia de resultados"
            )

    def _create_suggestion_for_area(
        self, query: str, intent_type: IntentType, area: str
