...
from src.chains.prompt_templates import EnhancedPromptTemplateSelector, TemplateMetadata
...
class TemplateOrchestrator:
    ...
    def __init__(self):
        ...
        self.confidence_threshold = getattr(settings, 'template_confidence_threshold', 0.7)
    ...
    def select_template(self, 
                       ...
                       base_prompt: str = "") -> TemplateSelectionResult:
        """Selecciona template óptimo basado en múltiples factores"""
        start_time = time.perf_counter()
        
        try:
            strategy = self._determine_selection_strategy(intent_result, query_complexity)
            ...
            if strategy == TemplateSelectionStrategy.INTENT_BASED:
                ...
                result = self._select_by_confidence(intent_result, user_expertise, base_prompt)
            elif strategy == TemplateSelectionStrategy.HYBRID:
                result = self._select_hybrid(intent_result, user_expertise, query_complexity, base_prompt)
            else:
                result = self._select_fallback(intent_result, user_expertise, base_prompt)
    ...
    def _determine_selection_strategy(self, intent_result: IntentResult, query_complexity: float) -> TemplateSelectionStrategy:
        """Determina la estrategia óptima de selección"""
        if intent_result.fallback_used or intent_result.intent_type == IntentType.UNKNOWN:
            return TemplateSelectionStrategy.FALLBACK
        
        if intent_result.confidence >= 0.9:
            return TemplateSelectionStrategy.INTENT_BASED
        
        if intent_result.confidence >= self.confidence_threshold and query_complexity > 0.6:
            return TemplateSelectionStrategy.CONFIDENCE_WEIGHTED
        
        return TemplateSelectionStrategy.HYBRID
    
    def _select_by_intent(self, intent_result: IntentResult, user_expertise: str, base_prompt: str) -> TemplateSelectionResult:
        ...
        template_metadata = self.template_selector.get_template_metadata(intent_result.intent_type)
    ...
    def _select_by_confidence(self, intent_result: IntentResult, user_expertise: str, base_prompt: str) -> TemplateSelectionResult:
        ...
        if user_expertise != "intermediate":
            adapted_prompt = self.template_selector._adapt_template_for_expertise(adapted_prompt, user_expertise)
        
        return TemplateSelectionResult(
            template_prompt=adapted_prompt,
            template_metadata=self.template_selector.get_template_metadata(IntentType.UNKNOWN),
    ...
    def _select_hybrid(self, intent_result: IntentResult, user_expertise: str, query_complexity: float, base_prompt: str) -> TemplateSelectionResult:
    ...
    def _calculate_hybrid_score(self, intent_result: IntentResult, query_complexity: float) -> float:
    ...
    def _select_fallback(self, intent_result: IntentResult, user_expertise: str, base_prompt: str) -> TemplateSelectionResult:
        """Selección de fallback cuando otras estrategias fallan"""
        fallback_prompt = base_prompt if base_prompt else self.template_selector._get_default_academic_template()
        
        if hasattr(self.template_selector, '_adapt_template_for_expertise'):
            fallback_prompt = self.template_selector._adapt_template_for_expertise(fallback_prompt, user_expertise)
    ...
    def _create_fallback_result(self, base_prompt: str, error_reason: str, processing_time: float) -> TemplateSelectionResult:
    ...
    def _update_metrics(self, result: TemplateSelectionResult, processing_time: float):
        ...
        self.selection_metrics["average_processing_time"] = (current_avg * (total - 1) + processing_time) / total
    
    def get_performance_metrics(self) -> Dict[str, Any]:
        """Obtiene métricas de performance del orchestrator"""
        total = self.selection_metrics["total_selections"]
        if total == 0:
            return {"status": "no_data", "message": "No template selections recorded yet"}
        ...
        return {
            ...
            "average_processing_time_ms": round(self.selection_metrics["average_processing_time"], 2),
